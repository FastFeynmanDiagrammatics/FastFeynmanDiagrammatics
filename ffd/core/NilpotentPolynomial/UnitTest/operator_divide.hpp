namespace ffd::nilpotent_polynomial::unit_test{
  ...
  template<SetT SetType=Full,
	   bool print=false>
  bool test_divide(int order, Real factor, int omega1, int omega2, int delta1, int delta2){
    bool IsOk = true;
    NilpotentPolynomial<long double, SetType> P(order), Q(order), AP(order), AQ(order);
    for(int j=0; j < 1<<order; ++j){
      if (SetType==Full ||
	  (SetType==Even && __builtin_parity(j)==0)){
	// || (SetType==Odd && __builtin_parity(j)==1)
	...
	std::cerr 
		  <<	std::abs(P[j] - P1[j]) << ' '
		  << 	(std::abs(AL[j])+std::abs(AP[j])+std::abs(AQ[j]))
		  << '\n';
      }
      IsOk = IsOk &&
	std::abs(P[j] - P1[j])/
	(std::abs(AL[j])+std::abs(AP[j])+std::abs(AQ[j])+
	 std::numeric_limits<long double>::min()) < factor*std::numeric_limits<long double>::epsilon();
    }
    return IsOk;
  }
  ...
  void operator_divide(){
    // test_divide<Full, true>(k, factor, 23, 7, -816, -192);
    for(int k=0; k<12; ++k){
      long double factor = 10*pow(3,k);

      assert((test_divide<>(k, factor, 3, 5, -12, -123)));
      assert((test_divide<>(k, factor, 13, 11, -532, -3283)));
      assert((  test_divide<>(k, factor, 17, 19, -216, -60192)));
      assert((test_divide<>(k, factor, 23, 7, -816, -192)));
      
      assert((test_divide<Even>(k, factor, 3, 5, -12, -123)));
      assert((test_divide<Even>(k, factor, 13, 11, -532, -3283)));
      assert((test_divide<Even>(k, factor, 17, 19, -216, -60192)));
      assert((test_divide<Even>(k, factor, 23, 7, -816, -192)));
