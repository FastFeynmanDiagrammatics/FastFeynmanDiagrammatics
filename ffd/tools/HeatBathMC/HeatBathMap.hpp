namespace ffd::heat_bath_mc{
  ...
  auto
  HeatBathMap(
        std::vector<Field> const& map_matrix,
        poly_t const& P,
	      seed_t const& Phi,
    ...
    // auto const [times, chosen_set] = heat_bath_fluxes_map<max_order, max_calculated_order>(fluxes, map_matrix);
    auto const [times, chosen_set] = heat_bath_fluxes_map_post<max_order, max_calculated_order>(fluxes, map_matrix);
