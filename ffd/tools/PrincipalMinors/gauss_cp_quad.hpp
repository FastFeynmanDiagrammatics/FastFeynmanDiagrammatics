...
Gauss_CP_quad(matrix_t&& matrix, BinaryInt step = 1) {
  ...
  const int linear_size =
      ffd::core_math::sqrt_int(size(matrix) / (step * step));
  ...
  for (uint set = 1; set < cardinality; ++set) {
    ...
    if (pop_set <= max_ord) {
      ...
      for (uint v_in = 0; v_in < linear_size * step; ++v_in) {
        ...
        if (set_ & (1 << v_in)) {
          for (uint v_ou = 0; v_ou < linear_size * step; ++v_ou) {
            if (set_ & (1 << v_ou)) {
              minor_matrix[w_in * minor_size + w_ou] =
                  matrix[v_in * linear_size * step + v_ou];
              w_ou++;
            }
          }
          w_in++;
      ...
      minors[set] = Gauss_CP_quad(minor_matrix);
    }
  }
  return minors;
}
