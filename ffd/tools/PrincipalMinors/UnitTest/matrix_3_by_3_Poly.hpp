  ...
  void matrix_3_by_3_Poly(){
   ...
   // exit(1);
    ...
    int cardinality = (1<<lin_size);
  	...
  	std::vector<NilpotentPolynomial<Real>> Rslt_dets(cardinality,NilpotentPolynomial<Real>(lin_size));

  	for (int element : Range(Mtx.size())){
      for (int set : Range(cardinality)){
  		  Mtx[element][set] = 2.0 * cos(set+1.3315*element+6.3162) -1.0;
  	...
  	Rslt_dets = DeterminantPrincipalMinors(Mtx);
  	for (int minor : Range(1,Rslt.size())){
      for (int set : Range(cardinality)){
    		// std::cerr << minor << " " << set << " " << Rslt[minor][set] <<" " <<  Rslt_minors[minor][set] << std::endl;
  	 	  assert(std::abs(Rslt[minor][set] - Rslt_dets[minor][set]) <1e-10);
      }
    }
  }
}//namespace
