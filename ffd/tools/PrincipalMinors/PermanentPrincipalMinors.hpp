...
namespace ffd::principal_minors{
	...
	PermanentPrincipalMinors(matrix_t&& matrix, BinaryInt step=1){

    using namespace ffd::permanent;
    ...
    for (uint set = 1; set < cardinality; ++set){
      ...
      uint w_in = 0;
      uint minor_size = __builtin_popcount(set_);
      ...
      minors[set] = Permanent(minor_matrix);
    }
