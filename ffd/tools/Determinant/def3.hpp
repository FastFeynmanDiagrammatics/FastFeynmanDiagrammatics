namespace ffd::determinant{
  ...
  Determinant(matrix_t&& M_){
    ...
    default:
      ...
      for(std::size_t j=0; j<lin_size-3; ++j){
	...
	for(std::size_t k=j+1; k < lin_size; ++k){
      ...
      return det*(
		  M_[lin_size-3+lin_size*(lin_size-3)]
		  * (M_[lin_size-2+lin_size*(lin_size-2)]*M_[lin_size-1+lin_size*(lin_size-1)] -
		     M_[lin_size-1+lin_size*(lin_size-2)]*M_[lin_size-2+lin_size*(lin_size-1)]) -
		  M_[lin_size-2+lin_size*(lin_size-3)]
		  * (M_[lin_size-3+lin_size*(lin_size-2)]*M_[lin_size-1+lin_size*(lin_size-1)] -
		     M_[lin_size-1+lin_size*(lin_size-2)]*M_[lin_size-3+lin_size*(lin_size-1)]) +
		  M_[lin_size-1+lin_size*(lin_size-3)]
		  * (M_[lin_size-3+lin_size*(lin_size-2)]*M_[lin_size-2+lin_size*(lin_size-1)] -
		     M_[lin_size-2+lin_size*(lin_size-2)]*M_[lin_size-3+lin_size*(lin_size-1)])
		  );
    }
  }
