...
int main(int argc, char* argv[]) {
  bool no_hold = true, verbose = false, fix_density = false,
       trust_errorbar = true, reweight_with_errorbars = true,
       exact_zeroth_order = true;
  ...
  for (ulong j = 1; j < argc; ++j) {
    ...
    if (!no_hold)
      ...
      if (arg == "-v") {
        ...
        verbose = true;
      } else if (arg == "-f" || arg == "--fix-density") {
        std::cout << "fix density mode activated\n";
        fix_density = true;
      } else if (arg == "-nf" || arg == "--no-fix-density") {
        fix_density = false;
      } else if (arg == "-nw" || arg == "--no-reweight" ||
                 arg == "--no-reweighting") {
        std::cout << "No-reweight mode activated" << std::endl;
        reweight_with_errorbars = false;
      } else if (arg == "-nt" || arg == "--discard-errorbar" ||
                 arg == "--empirical-errorbar") {
        std::cout << "!!! discarding Monte Carlo error bar (empirical mode) "
                     "not yet implemented"
                  << std::endl;
        trust_errorbar = false;
      } else if (arg == "-e" || arg == "--exact-zeroth-order") {
        std::cout
            << "the zeroth order is exactly given (no need to normalize it)\n";
        exact_zeroth_order = true;
      } else if (arg == "-ne" || arg == "--normalize-exact-zeroth-order") {
        std::cout << "the zeroth order needs normalization\n";
        exact_zeroth_order = false;
      } else if (arg == "-t" || arg == "--trust-errorbar" ||
                 arg == "--use-errorbar") {
        trust_errorbar = true;
      } else {
  ...
  auto compute_mean_dir = [&file_extension, trust_errorbar,
                           reweight_with_errorbars, exact_zeroth_order,
    ...
    std::map<std::pair<int, int>, std::array<val_err_t, max_order>> mp_emp;
    std::map<std::pair<int, int>, std::array<val_err_t, max_order>> mp_count;
    std::map<std::tuple<int, int, int>, wmean_t> mp_d;
    std::map<std::tuple<int, int, int>, val_err_t> mp_emp_d;
    std::map<std::tuple<int, int, int>, val_err_t> mp_count_d;
        ...
        //	  std::cerr << "pid = " << pid;
        {
          ...
          while (df >> order0 >> order1 >> value >> error) {
            mp_d[std::tuple<int, int, int>{pid, order0, order1}] &=
                wmean_t(value, error);
            mp_emp_d[std::tuple<int, int, int>{pid, order0, order1}] +=
                val_err_t(value, error);
            mp_count_d[std::tuple<int, int, int>{pid, order0, order1}] += 1.;
          }
        }
    ...
    if (!mp.empty()) {
      ...
      if (reweight_with_errorbars) {
        for (auto [key, val] : mp) {
          auto [pid, order] = key;
          order_max = std::max(order, order_max);
          if (exact_zeroth_order)
            merged[0] = val[1];
          for (ulong j = exact_zeroth_order; j < order + 1; ++j) {
            auto const val_j =
        ...
        for (auto [key, val] : mp_emp) {
          auto [pid, order] = key;
          order_max = std::max(order, order_max);
          if (exact_zeroth_order)
            merged[0] &= wmean_t((val[1] / mp_count[key][0]).eval());
          for (ulong j = exact_zeroth_order; j < order + 1; ++j) {
            auto const val_j =
                val_err_t(val[j + 1].eval()) / val_err_t(val[0].eval());
            merged[j] &= wmean_t(val_j.eval());
      ...
      if (reweight_with_errorbars) {
        for (auto [key, val] : mp_d) {
          ...
          if (order0 != -1 && order1 != -1) {
            ...
            if ((order0 != 0 || !exact_zeroth_order) || order1 != 0)
              val_temp = val_temp /
                         val_err_t(mp_d[std::make_tuple(pid, -1, -1)].eval());
            merged[std::pair<int, int>{order0, order1}] &=
                wmean_t(val_temp.eval());
          }
        }
      } else {
        for (auto [key, val] : mp_emp_d) {
          auto [pid, order0, order1] = key;
          if (order0 != -1 && order1 != -1) {
            auto val_temp = val;
            if ((order0 != 0 || !exact_zeroth_order) || order1 != 0)
              val_temp =
                  val_temp / val_err_t(mp_emp_d[std::make_tuple(pid, -1, -1)]);
            merged[std::pair<int, int>{order0, order1}] &=
                wmean_t(val_temp.eval());
          }
        }
