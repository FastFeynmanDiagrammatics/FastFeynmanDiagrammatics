...
void CDet_Sigma_r(
    vector3d<field>& __restrict__ Sigma,
    vector3d<field> const& __restrict__ rho_T,
    array2d<std::uint8_t, n + 1, (1ul << n)> const& __restrict__ bm) {
  assert((size_x(Sigma) == n));
  assert((size_y(Sigma) == n));
  assert((size_z(Sigma) == (1ul << n)));
...
#pragma GCC ivdep
          for (ulong l = 0; l < bm_0_S; ++l) {
            ...
            acc += Sigma[S_n_j_n + b_l] * rho_T[V_S_n_k + b_l];
